    ...
    */
    ...
    public static void testSonucu(char[][] answers, char[] keys) {
        ...
        for (int i = 0; i < answers.length; i++) { // her bir ogrenciye bakar, i--> ogrenci
            int count = 0; // her bir ogreninin dogru cevabini sayacagi icin, sayac buraya konur
            for (int j = 0; j < answers[i].length; j++) { // her ogrencinin cevaplarina bakar, j--> ogrenci cevaplari
                if (answers[i][j] == keys[j]) { // cevap anahtari ile ogrenci cevaplari karşilaştiriliyor
                    count++;
                }
            }
